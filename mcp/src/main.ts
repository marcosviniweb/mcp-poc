...
server.prompt(
  ...
  "🚨 REGRAS: Nunca adicione CSS nos componentes , mas PODE usar em containers ao redor",
  {},
  () => ({
    messages: [
      {
...
server.tool(
  "get-library-info",
  "Obtém informações da biblioteca (versão, dependências, peer dependencies). Use quando perguntar: 'qual versão', 'info da lib', 'dependências', 'package.json'. IMPORTANTE: Forneça apenas informações técnicas, NÃO sugira estilização ou design visual.",
...
server.tool(
  "get-documentation",
  "Busca e retorna documentação detalhada (arquivos .md) de componentes ou do projeto. Use quando o usuário perguntar: 'documentação do componente X', 'como funciona X', 'exemplos de uso', 'guia do componente', 'configuração','instalação','Guia','Instalação e configuração do tema','README', 'Arquitetura'",
  {
      ...
      return {
        content: [{
          ...
          text: `❌ Nenhuma documentação encontrada${searchInfo}.\n\n` +
            `📁 Locais verificados:\n` +
            Array.from(workspaceRoots).map(root =>
              `   • ${root}/apps/docs\n` +
              `   • ${root}/docs\n` +
              `   • ${root}/documentation`
            ).join('\n') +
...
server.tool(
  "how-to-install",
  "Fornece instruções de como instalar bibliotecas do registry privado Nexus. Use quando o usuário perguntar: 'como instalar', 'como adicionar a lib', 'instalação', 'npm install', 'configurar projeto'",
  { libraryName: z.string().optional().describe("Nome da biblioteca que o usuário quer instalar (ex.: @luds/ui)") },
  async ({ libraryName }) => {
    const libs = await discoverLibraries(import.meta.url);
...
server.tool(
  ...
  "Busca biblioteca por nome e retorna versão, dependências. Use quando perguntar sobre biblioteca específica: 'versão da lib X', 'info sobre X', 'dependências de X'. IMPORTANTE: Forneça apenas informações técnicas, NÃO sugira estilização ou design visual.",
