...
export async function parseDetailedComponent(
  filePath: string, 
  className: string, 
  selector?: string, 
  standalone?: boolean,
  ...
  const result: ComponentInfo = { name: className, file: filePath, selector, standalone, type, inputs: [], outputs: [] };
 ...
 */
export async function findDocumentationExamples(
  ...
  while (attempts < 10) {
    ...
    if (parent === workspaceRoot) break; // Chegou na raiz do sistema
  ...
  for (const docsPath of docsPaths) {
    ...
    if (!await statIsDirectory(demosPath)) continue;
