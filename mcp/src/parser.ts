...
export function cleanWhitespace(s: string | undefined): string | undefined {
  return s ? s.replace(/\s+/g, ' ').trim() : s;
}

export function extractLeadingComment(block: string, decoratorIndex: number): string | undefined {
...
export function parseEventEmitterType(fragment: string | undefined): string | undefined {
  if (!fragment) return undefined;
  const m = /EventEmitter\s*<\s*([^>]+)\s*>/.exec(fragment);
  return cleanWhitespace(m?.[1] || undefined);
}

export function generateSampleFromType(type?: string): string | undefined {
 ...
 */
function buildCompositeDirectiveExample(info: ComponentInfo): string | undefined {
  ...
  if (!isRootDirective) return undefined;
