...
const PUBLIC_API_RELATIVE = path.join("projects", "my-lib", "src", "public-api.ts");
...
async function parseReExports(fileContent) {
...
async function resolveTsPath(baseFile, fromPath) {
...
async function collectExportChain(entryFile, visited = new Set()) {
    const result = [];
...
async function listPotentialComponentFiles() {
...
server.tool("get-component", "Obtém detalhes de um componente pelo nome da classe", { name: z.string().min(1).describe("Nome da classe do componente, ex.: ButtonComponent") }, async ({ name }) => {
    const files = await listPotentialComponentFiles();
